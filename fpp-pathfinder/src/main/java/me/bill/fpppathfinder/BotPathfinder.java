...
public final class BotPathfinder {
    ...
    while (!open.isEmpty() && explored++ < nodeLimit) {
      ...
      for (int[] nb : neighbors(world, cur.pos().x(), cur.pos().y(), cur.pos().z(), opts)) {
        ...
        if (existing == null || newG < existing) {
          ...
          open.add(
              new Node(
                  new Pos(nb[0], nb[1], nb[2]),
                  cur,
                  newG,
                  heuristic(nb[0], nb[1], nb[2], goal),
                  mt));
        }
      }
    }
  ...
  private static int heuristic(Pos a, Pos b) {
    return heuristic(a.x(), a.y(), a.z(), b);
  }

  private static int heuristic(int ax, int ay, int az, Pos b) {
    int dx = Math.abs(ax - b.x());
    int dy = Math.abs(ay - b.y());
    int dz = Math.abs(az - b.z());
