...
- `BotMessageConfig`
- `/fpp chat`
- fake chat event triggers and bot-to-bot/player-reaction logic
- `bot-messages.yml` and chat message pool syncing

Core should not generate `bot-messages.yml`, expose chat tier commands, or print fake-chat startup status. If LuckPerms prefixes are needed in chat output, depend on the LuckPerms extension/API path rather than direct core helpers.
